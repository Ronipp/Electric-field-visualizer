...
arr = np.loadtxt('mittaus.csv', delimiter=', ')
jana = []
janax = []
    ...
    for rivi in matrix:
        for solu in rivi:
            ...
            if (x > 0 and x < len(rivi) - 1) and (y > 0 and y < len(matrix) - 1):
                ...
                if solu.y == 10 and 4 < solu.x < 24:
                    jana.append(solu.sähkösuuruus)
                    janax.append(solu.x-4)
                # if solu.y == 10 and solu.x > 10:
                #     jana.append(solu.mittaus)
                #     janax.append(solu.x)
...
def func(x, a, b):
    return a*(np.log(x)-np.log(abs(x-20)))+b


x = np.linspace(2, 18, 300)
yy = 1.708*(1/x - 1/(x-20))
o, u = curve_fit(func, janax, jana)
print(u)
aa = round(o[0], 3)
bb = round(o[1], 3)
plt.plot(janax, jana, marker='o', label='kokeelliset sähkökentät')
# plt.plot(x, func(x, *o), label=('%s(ln(x) - ln|x-20|) + %s') % (aa, bb))
plt.plot(x, yy, label='y = 1.708*(1/x - 1/(x-20))')
plt.legend()
plt.xlabel('Etäisyys (cm)')
plt.ylabel('Sähkökentän suuruus (V/cm)')
plt.show()
...
while not valmis:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_p:
            pg.image.save(screen, 'sähkö'+str(p)+'.png')
            p += 1
        ...
        for rivi in solulista:
            for sol in rivi:
                if sol.sähkövektori:
                    ...
                    loppu = [int((alku[0] + 20*sol.sähkövektori[0])),
                             int((alku[1] + 20*sol.sähkövektori[1]))]
                    if sol.mittaus <= 1:
                        väri = [0, 0, 255]
                    ...
                    pg.draw.circle(screen, väri, loppu, 4)
